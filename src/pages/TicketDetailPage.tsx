...
import { addComment, getComments , updateComment , deleteComment} from '@/services/comment.service';
...
export default function TicketDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchTicket = async () => {
      try {
        ...
        // Fetch comments separately to refresh them
        const comments = await getComments(id);
        // console.log("comments" , comments.comments);

        setTicket(prev => prev ? { ...prev, comments: comments.comments } : prev);
        // console.log(ticket);
  ...
  const handleAddComment = async () => {
    ...
    try {
      await addComment(ticket.id.toString(), newComment);
      // Refresh comments to reflect immediately
      const updatedComments = await getComments(ticket.id.toString());
      setTicket(prev => prev ? { ...prev, comments: updatedComments.comments } : prev);
    ...
    // await api.put(`/tickets/comments/${commentId}`, {
    //   content: editedContent
    // });

    // setEditingCommentId(null);
    // setEditedContent("");
    // refetchTicket(); // or update local state
  ...
  const handleUploadAttachment = async () => {
    ...
    try {
      ...
      setTicket(prev => prev ? { ...prev, attachments: [...prev.attachments, ...uploadedAttachments] } : prev);
  ...
  const isImageFile = (fileName: string) => {
    // console.log(fileName.toLowerCase().endsWith('.jpg'));
  ...
  const handleStatusChange = async (newStatus: string) => {
    try {
        const ticketUpdatStatus =  await updateTicketStatus(ticket.id, newStatus as TicketStatus);
          
        // console.log(ticketUpdatStatus);

        setTicket(prev =>
  prev ? { ...prev, ...ticketUpdatStatus } : prev
);
      toast({
        title: "Status Updated",
        description: `Ticket status changed to ${newStatus.replace("_", " ")}`,
      ...
      // Refresh ticket to reflect changes immediately
      // const updatedTicket = await getTicketById(ticket.id.toString());
      // setTicket(updatedTicket);

      // // Refresh comments separately to ensure they're up to date
      // const comments = await getComments(ticket.id.toString());
      // setTicket(prev => prev ? { ...prev, comments: comments.comments } : prev);
    } catch (error) {
      toast({
        title: "Error",
                  ...
                  <div className="flex justify-center">
                    {previewAttachment?.fileUrl && (
                      <img
                        ...
                        onError={(e) => {
                          console.error('Failed to load image:', previewAttachment.fileUrl);
                  ...
                  {ticket.attachments.map(att => (
                    <div key={att.id} className="flex items-center justify-between p-3 bg-muted/30 border border-border">
                      <div className="flex items-center gap-2">
                        ...
                        {isImageFile(att.fileName) && att.fileUrl ? (
                          <button
